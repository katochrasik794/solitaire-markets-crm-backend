...
router.get('/users/all', authenticateAdmin, async (req, res, next) => {
  try {
    ...
    // Note: users table doesn't have a status column yet
    // For now, we'll filter in memory if status is provided
    // TODO: Add status column to users table if needed
    // if (status) {
    ...
    // Filter by status in memory (since users table doesn't have status column yet)
    if (status) {
      const statusLower = String(status).toLowerCase();
      if (statusLower === 'banned') {
        // For now, no users are banned since there's no status column
        items = [];
      } else if (statusLower === 'active') {
        // All users are active by default
        // items remain as is
      } else if (statusLower === 'inactive') {
        // No inactive users for now
        items = [];
      }
    }
...
router.patch('/users/:id', authenticateAdmin, async (req, res, next) => {
  try {
    ...
    const query = `
      ...
      SET ${fields.join(', ')}
      WHERE id = $${idx}
      RETURNING id, email, first_name, last_name, phone_code, phone_number, country, is_email_verified, created_at, updated_at
    `;
    ...
    result.rows.forEach(row => {
      ...
      if (!usersMap[userId]) {
        ...
        usersMap[userId] = {
          ...
          totalBalance: 0, // Will be calculated from MT5 API if needed
